...
esp_err_t app_nvs_save_sta_creds(void)
{
    nvs_handle handle;
    esp_err_t esp_err;
    ESP_LOGI(TAG, "app_nvs_save_sta_creds: Saving station mode creds to flash");
    ...
    if (wifi_sta_config)
    {
        ...
        if (esp_err != ESP_OK)
        {
            printf("app_nvs_save_sta_creds: Error (%s) apening NVS handle!\n", esp_err_to_name(esp_err));
            return esp_err;
        }
...
bool app_nvs_load_sta_creds(void)
{
    ...
    ESP_LOGI(TAG, "app_nvs_load_sta_creds: Loading Wifi credential from flash");
