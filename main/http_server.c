...
/*
Checks the g_fw_update_status and create the fw_update_reset timer if g_fw_update_status is true.
*/
...
/*
HTTP server monitor task used to track event of the HTTP server
@param pvParameters: parameter whcih can be passed to the task
*/
        ...
        if (xQueueReceive(http_server_monitor_queue_handle, &msg, portMAX_DELAY))
        {
            switch (msg.msgID)
            {
                ...
                case HTTP_MSG_WIFI_CONNECT_SUCCESS:
                    ...
                    g_wifi_connect_status = HTTP_WIFI_STATUS_CONNECT_SUCCES;
                    break;
...
/*
Jquet get handler is requested when accessing the webpage.
...
/*
Sends the .ico file wwhen accessing the webpage
@param req: HTTP request for which the uri needs to be handled.
@return ESP_OK
*/
...
@param  req: HTTP request for chich the uri needs to be handles.
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        ...
        printf("http_server_OTA_updat_handler: OTA RX: %d of %d\r", content_received, content_length);

        // Is this the first data we are receiving 
        // If so, it willl have the information in the header that we need
        if(!is_req_body_started)
        {
            ...
            char *body_start_p = strstr(ota_buff, termination_code) + strlen(termination_code);
            int body_part_len = recv_len - (body_start_p - ota_buff);

            printf("http_server_OTA_updat_handler: OTA file size: %d\r\n", content_length);
            esp_err_t err = esp_ota_begin(update_partition, OTA_SIZE_UNKNOWN, &ota_handle);
            if (err != ESP_OK)
            {
                printf("http_server_OTA_update_handler: Errog with OTA begin, cancelling OTA\r\n");
                return ESP_FAIL;
            }
            else
            {
    ...
    // Wont update the global variable through the file, so send the message about the status
    if (flash_successful) 
    {
...
/*
DHT snsor readings JSON handler responds with DHT22 sensor data
@param req: HTTP request for which the uri needs to be handled
@return ESP_OK
*/
...
/*
WifiConnect.json handler is invoked after the connect button is pressed and handles receiving the SSID
and pssword entered by the user
@param req: HTTP request for which the uri needs to be handled
@return ESP_OK
*/
static esp_err_t http_server_wifi_connect_json_handler(httpd_req_t *req)
{
    ...
    size_t len_ssid = 0, len_pwd = 0;
    char *ssid_str = NULL, *pwd_str = NULL;
    ...
    // Update the Wifi networks confifuration and let the wifi app know
...
/*
WifiConnect Status handler updates the connection status for the web page
*/
static esp_err_t http_server_wifi_connect_status_json_handler(httpd_req_t *req)
{
    ESP_LOGI(TAG, "/wifiConnectStatuc requested");
...
/*
WifiConnectInfo.json handle updates the web page with conneciton information.
*/
static esp_err_t http_server_get_wifi_connect_info_json_handler(httpd_req_t *req)
{
    ...
    if (g_wifi_connect_status == HTTP_WIFI_STATUS_CONNECT_SUCCES)
    {
...
/*
wifiDisconnect.json responds by sending a message to the wifi application to disconnect.
*/
...
static httpd_handle_t http_server_configure(void)
{
    ...
    if (httpd_start(&http_server_handle, &config) == ESP_OK)
    {
        ...
        // register wifiConnect.json handler
        httpd_uri_t wifi_connect_satus_json = {
            .uri = "/wifiConnectStatus",
            .method = HTTP_POST,
            .handler = http_server_wifi_connect_status_json_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &wifi_connect_satus_json);
