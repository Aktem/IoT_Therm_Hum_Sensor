...
// Used to track the number for retries when a connection attemp fails
...
@param arg: asife from event data, that is passed to the handler when it is called
...
/*
Init the TCP stack and default wifi config
*/
static void wifi_app_default_wifi_init(void)
{
    ...
    // Default wifi config - ops must be in this order
...
/*
Config the Wifi acces point setting ans assign the static IP to the SoftAP
*/
static void wifi_app_soft_ap_config(void)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_set_bandwidth(WIFI_IF_AP, WIFI_AP_BANDWIDTH)); // defualt bandwidth of 20MHz
    ESP_ERROR_CHECK(esp_wifi_set_ps(WIFI_STA_POWER_SAVE));  // power save set to "NONE"
}


/*
Inits tha Wifi application eventy handler for wifi and ip events.
*/
static void wifi_app_event_handler_init(void)
{
    //Evnet loop for the Wifi driver
...
@param pvParameters: param which can be passed to the task
*/
        ...
        if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
        {
            switch (msg.msgID)
            {
                ...
                case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
                    ...
                    // Set current number of retreis to 0
                    g_retry_number = 0;

                    // Let the http serve know about the connection attempt
                ...
                case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
                    ...
                    if (eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT) // save STA creds only if conn. from HTTP server
                    {
                        xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT); // Clear the bits, in case we want to discon. and reconnect
                    }
                    else
                    {
                ...
                case WIFI_APP_MSG_USER_REQUESTED_STA_DISCONNECT:
                    ...
                    if (eventBits & WIFI_APP_STA_CONNECTED_GOT_IP_BIT)
                    {
                        ...
                        // So don't try and reconnect when the disconnect button is called.
                        g_retry_number = MAX_CONNECTION_RETRIES;
                        ESP_ERROR_CHECK(esp_wifi_disconnect());
                        app_nvs_clear_sta_creds();
                        rgb_led_http_sever_started(); // name dosn't really make since any more 
                    }
                ...
                case WIFI_APP_MSG_STA_DISCONNECTED:
                    ...
                    {
                        ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECTED: ATTEMPT USING SAVE CREDENTIALS");
                    ...
                    {
                        ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECTED: USER REQUESTED DISCONECTION FROM HTTP SERVER");
                    ...
                    {
                        ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECTED: ATTEMPT FAILED, CHECK WIFI ACCESS AVAILABILITY"); // Could also keep trying to connect
                    }
...
void wifi_app_start(void)
{
    ...
    // Allocate mem for the wifi config
